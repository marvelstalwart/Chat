...
export default function SetAvatar() {
  ...
  <div className='w-screen h-screen flex flex-col gap-2 bg-blue-100 justify-center '>
    ...
    <div className='font-bold text-2xl text-center'>{isError?<div className='color-red-400'>{message}</div>:<>Carry one Avatar for here</>}</div>
    <div className=''>
   ...
   <div className="flex justify-center gap-2">
    {avatars && avatars.map((avatar, index)=> {
      ...
      <div className='w-screen flex  justify-center mt-2 gap-1'>
         <button className='p-2 text-xs rounded-lg text-white font-medium  w-fit bg-sky-500/100 hover:bg-sky-500/80 cursor-pointer' onClick={setProfilePicture}>Use dis one</button>
     
      <button onClick={()=> dispatch(getAvatars())} className='p-2 rounded-lg text-xs text-white font-medium  w-fit bg-sky-500/100 hover:bg-sky-500/80 cursor-pointer'>I no like dis type</button>
